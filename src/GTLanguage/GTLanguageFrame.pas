...
uses gnugettext, GTLanguageList, GTForm, common{, ConfigManagerUnit, dzDFASDefine};

procedure TGTfraLanguage.LanguageSelect();
var
  currCompo : integer;
  st : string;
  LangID: Word;
begin
  ...
  begin
    ...
    if LangID = 1042 then
    begin
      LanguageFlag := LANGUAGE_KOREA;
    end else if LangID = 1041 then
    begin
      LanguageFlag := LANGUAGE_JAPANESE;
    end else
    begin
  ...
  cmbLanguage.ItemIndex := LanguageFlag;


  // change language
  st := LanguageList.Code[cmbLanguage.Items[cmbLanguage.ItemIndex]];
  UseLanguage(st);
...
var
  ...
  st : string;
begin
  // 2012.06.11 jkchange : config파일에 언어 설정 저장.
  //TConfigManager.GetObj.LanguageFlag := cmbLanguage.ItemIndex;
end;
